...
pub async fn add_pr(id: String, title: String, data: String, status: String, inference_data: InferenceResponse) -> AppResult<()> {
    let db = DB.get().ok_or(anyhow::anyhow!(""))?;

    sqlx::query!(
        r#"
            INSERT INTO pr (id, title, data, status, score, opened_at)
